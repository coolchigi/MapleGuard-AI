...
Env vars (all optional; absence = offline dev):
  ...
  MAPLEGUARD_SESSION_BACKEND  file | s3 | none              (default file)
...
class Deployment:
    ...
    def is_offline(self) -> bool:
        ...
        return self.memory_backend in ("dev", "none") and self.session_backend in ("file", "none")
...
def build_session_manager(session_id: str, config: Deployment) -> Optional[Any]:
    ...
    file -> FileSessionManager (offline, dev/demo). s3 -> S3SessionManager (needs AWS). The
    demo uses file; the S3 swap is one field. Verified against SDK 1.53.0.
    """
