...
def ensure_knowledge_base(bedrock, region: str, kb_name: str, role_arn: str,
                          index_arn: str) -> str:
    # Reuse the KB by name if it already exists, so a re-run points at the existing KB (the live
    # one is HPV87UFYFI) instead of creating a duplicate. create_knowledge_base does not dedupe.
    paginator = bedrock.get_paginator("list_knowledge_bases")
    for page in paginator.paginate():
        for kb in page.get("knowledgeBaseSummaries", []):
            if kb.get("name") == kb_name:
                _log("kb", f"knowledge base {kb['knowledgeBaseId']} already exists, reusing")
