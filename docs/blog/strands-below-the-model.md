...
MapleGuard's job is to do the repetitive work of tracking a person's Canadian immigration position on its own: recompute where they stand when the rules or draws move, surface the point drops coming on future dates, and audit a reference letter against the official occupation. The agent does that work unattended. It draws a line at exactly two irreversible actions, filing a government application and declaring someone eligible, and those it never takes for you.

MapleGuard is a Strands agent, and the interesting engineering is not that a model calls some functions. It is what Strands lets us build underneath the model so that work is both autonomous and safe. The split that makes it possible: the model orchestrates, retrieves, and explains, and it never computes a score or asserts eligibility. Deterministic code does. This post is about the Strands machinery that turns that split, and the line at the two irreversible actions, from lines in a prompt into something enforced in code.
...
A wrapper hands the model a task and trusts the prose it returns. MapleGuard uses Strands to do the opposite. The model is given typed tools and told to route to them, a pre-tool-call hook that cancels a forbidden action, a response screen that refuses an eligibility verdict, a memory layer that lets it cite a retrieved source without ever pulling a number from it, and a specialist split that keeps the same guarantees on both sides. The agent runs the whole loop on its own: it routes, retrieves, computes through the tools, and explains. The SDK is used, deliberately and at each layer, to make that autonomy safe, so the agent can do the work unattended and still never compute the number itself and never cross the line at the two irreversible actions. Doing the repetitive work without a human, and stopping cold at the one step that must stay human, is the product. Strands is how both halves are enforced.
