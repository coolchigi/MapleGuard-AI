...
- [x] **`sirs_bc(profile, offer)`** (`pnp/bc.py`) — BC SIRS 200-pt structure (work 40, edu 40, language 40, wage 55, area 25 = 120 human + 80 economic), +600-CRS nomination constant, `job_offer_required` + `eligible_to_register` flags, Tech-exempt handling. Structure/caps/flags tested.
  - [~] **Verify SIRS bands** — `server/pnp/sirs_ingest.py` fetch→parse→reconcile pipeline (AgentCore Browser fetch seam) makes the bands AUDITABLE: a band is `verified` only when the official source parses cleanly AND matches the in-code value; any disagreement surfaces as a cited mismatch (the fixture carries a planted discrepancy so the offline reconcile honestly reports `all_verified=False`, proving it compares rather than rubber-stamps). No SIRS numbers fabricated. The flip to `verified=True` stays a human step after a real live fetch confirms, so `verified=False` + the xfail remain intentional until then.
